...
from student_life_state_machine import student_life_state_machine
...
class StateMachineAction(LoopAction):
    ...
    def _create_unique_entity_mappings(self, domain: Domain) -> Set[Text]:
        ...
        for slot_mappings in domain.slot_mapping_for_form(
            self.name()
        ).values():
            for slot_mapping in slot_mappings:
                if slot_mapping.get("type") == str(SlotMapping.FROM_ENTITY):
                    mapping_as_string = json.dumps(
                        slot_mapping, sort_keys=True
                    )
                    if mapping_as_string in unique_entity_slot_mappings:
                        ...
                        mapping_as_string not in duplicate_entity_slot_mappings
                    ):
    ...
    def entity_is_desired(
        ...
        entity_mapping_is_unique = self._entity_mapping_is_unique(
            slot_mapping, domain
        )
        ...
        # is set)
        if (
            slot_mapping.get("role") is None
            and slot_mapping.get("group") is None
    ...
    async def _get_slot_fill_events(
        ...
        slot_filled_utterance: Optional[Utterance]
    ...
    async def _get_responses_events(
        ...
        valid_responses_action_names = [
            valid_response.actions[0]
            for valid_response in valid_responses.actions
        ]
        ...
        for action in valid_responses_actions:
    ...
    async def validate(
        ...
        state_machine_state: StateMachineState = (
            student_life_state_machine.student_life_state_machine
        )  # TODO

        # Using the last user message, fill slots
        events: List[Event] = []
        events += self._get_slot_fill_events(
        ...
        events += self._get_responses_events(
        ...
        events += self._check_transitions_events(
    ...
    async def request_next_slot(
        ...
        if not slot_to_request:
            slot_to_request = self._find_next_slot_to_request(
                temp_tracker, domain
            )
            request_slot_events.append(
    ...
    def _name_of_utterance(
        self, domain: Domain, slot_name: Text
    ) -> Optional[Text]:
        search_path = [
    ...
    async def _ask_for_slot(
        ...
        action_to_ask_for_next_slot = self._name_of_utterance(
            domain, slot_name
        )
    ...
    def _get_entity_type_of_slot_to_fill(
        ...
        mappings = [
            m
            for m in mappings
            if m.get("type") == str(SlotMapping.FROM_ENTITY)
        ]
